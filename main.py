...
class Pet(ABC):
    ...
    def __init__(self, name, update_status_callback):
        self.name = name
        self.hunger = 50
        self.happiness = 50
        self.training = 0
...
class GameManager:
    ...
    def setup_ui(self):
        ...
        tk.Button(self.button_frame, text="Quit", command=self.root.quit).grid(row=0, column=5)
    ...
    def choose_pet(self, choice):
        ...
        if choice == '1':
            self.pet = Dog(name, self.update_status)
        elif choice == '2':
            self.pet = Cat(name, self.update_status)
        elif choice == '3':
            self.pet = Dragon(name, self.update_status)
        elif choice == '4':
            self.pet = Unicorn(name, self.update_status)
        else:
            self.pet = Dog(name, self.update_status)
